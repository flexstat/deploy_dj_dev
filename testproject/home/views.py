...
def home(request):
	...
	if request.method == 'POST':
		...
		domainx = request.POST.get("domainx")
		...
		for subdomain in subdomains:
			work_donex ="[-]  {s}".format(s=subdomain)

		return HttpResponse("<h2>Domain target, {s}</h2>".format(s=subdomains),"<h2>Subdomains: , {0}</h2>".format(work_done))
